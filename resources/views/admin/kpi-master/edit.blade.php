            ...
            <div class="container-fluid pt-4 px-4">
                <div class="bg-secondary_1 border_light-1 rounded p-4">
                    <div class="d-flex align-items-center justify-content-between mb-4">
                        <h6 class="mb-0 text-color-1">Edit KPI Details</h6>
                    </div>
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                <div class="card-body">
                                    <div class="">
                                        <form class="needs-validation"  action="{{ route('kpi-master.update', $kpis->id) }}" id="kpi_master_form" method="post" enctype="multipart/form-data" novalidate>
                                            @csrf
                                            @method('PATCH')
                                            <div class="row">
                                                ...
                                                <div class="form-group col-md-4">
                                                    <label class="d-block mb-4">
                                                        <span class="form-label d-block mendatory">Agency *</span>
                                                        <select class="form-control" id="agency_id" name="agency_id" required>
                                                            <option value="">Select Agency </option>
                                                            @foreach ($agency as $data)
                                                                <option value="{{$data->id}}" @if($kpis->agency_id == $data->id) selected @endif>{{$data->name}}</option>
                                                            @endforeach
                                                        </select>
                                                        @error('agency_id')
                                                ...
                                                <div class="form-group col-md-4">
                                                    <label class="d-block mb-4">
                                                        <span class="form-label d-block mendatory">Events *</span>
                                                ...
                                                <input type="hidden" name="hidden_event_id" id="hidden_event_id" value="{{$kpis->event_id}}">

                                                <div class="form-group col-md-4">
                                                    <label class="d-block mb-4">
                                                        <span class="form-label d-block mendatory">KPI Name *</span>
                                                        <input class="form-control" id="kpi_name" type="text" name="kpi_name" placeholder="Enter KPI Name" autocomplete="kpi_name" value="{{$kpis->kpi_name}}" required>
                                                        @error('kpi_name')
                                                            <span class="text-danger">{{$message}}</span>
                                                        @enderror
                                                      </label>
                                                ...
                                                <div class="form-group col-md-4">
                                                    <label class="d-block mb-4">
                                                        <span class="form-label d-block mendatory">Description *</span>
                                                        <input class="form-control" id="description" type="text" name="description" placeholder="Enter Description" autocomplete="description" value="{{$kpis->description}}" required>
                                                        @error('description')
                                                            <span class="text-danger">{{$message}}</span>
                                                        @enderror
                                                      </label>
                                                ...
                                                <div class="form-group col-md-4">
                                                    <label class="d-block mb-4">
                                                        <span class="form-label d-block mendatory">Equation</span>
                                                        <input class="form-control" id="equation" type="text" name="equation" placeholder="Enter equation" autocomplete="equation" value="{{$kpis->equation}}">
                                                        @error('equation')
                                                            <span class="text-danger">{{$message}}</span>
                                                        @enderror
                                                      </label>
         ...
         $('#agency_id').on('change', function () {
            ...
            $.ajax({
                ...
                success: function (result) {
                    $("#event_id").html('<option value=" ">Select Events</option>');
                    $.each(result, function (key, value) {
                        selectedTxt = '';
                        if ($('#hidden_event_id').val() == value.id)
                            selectedTxt = 'selected';
                        $("#event_id").append('<option value="' + value.id + '" ' + selectedTxt + '>' + value.name + '</option>');
                    });
                }
            });
        });

        selectedAgencyId = ($('#agency_id').val());
